import { SafeAreaView, Text, View, StatusBar, Image, TouchableOpacity } from 'react-native'
...
const CartScreen = () => {
    ...
    const resturants = useSelector(selectedRestaurant)
    const cartItems = useSelector(selectCartItems)
    const cartTotal = useSelector(selectCartTotal)

    const dispatch = useDispatch()

    const [groupItems, setGroupItems] = useState<Array<any>>([])


    useMemo(() => {
        const gItems = cartItems.reduce((group: any, item: any) => {
        ...
        <View className="flex-1 pt-6 bg-white">
            ...
            <View className="relative py-4 shadow-sm">
                ...
                <View>
                    ...
                    <Text className="text-center text-gray-500">{resturants.name}</Text>
                </View>
                    ...
                    Object.entries(groupItems).map(([key, items]) => {
                        return (
                            <View key={key}
                                ...
                                <Image className="rounded-full h-14 w-14" source={items[0]?.image} />
                                <Text className="flex-1 font-bold text-gray-700">{items[0]?.name}</Text>
                                <Text className="text-base font-semibold">${items[0]?.price}</Text>
                                <TouchableOpacity
            ...
            {/* totals */}
            <View style={{ backgroundColor: themeColors.bgColor(0.2) }} className="p-6 px-8 space-y-4 rounded-t-3xl">
